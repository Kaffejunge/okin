...
class InteractiveTimeCourse:
    ...
    def find_stall_point(self, arr, tol=1e-6, f=1.05):     
        arr /= arr.max()
        stop = np.where(arr >= (1-tol) )[0]
        return int((stop[0]) * f)
    ...
    def _update_tc_plot(self):
        ...
        # P = solution[:, P_ind]
        self.t_max = self.find_stall_point(arr=solution[:, P_ind])
    ...
    def _update_vtna_plot(self):
        ...
        solution = odeint(self.ode_func, conc_state, t, args=(k_values,))
        P_ind = self.dynamic_species.index("P")
        self.t_max = self.find_stall_point(arr=solution[:, P_ind], tol=1e-5, f=1.1)
        ...
        for i, species in enumerate(self.dynamic_species):
            df1[species] = np.concatenate([
                                ...
                                solution1[700:1000:30, i] # last 300 points, every 30th
                            ])
        ...
        P_ind = self.dynamic_species.index("P")
        t_max2 = self.find_stall_point(arr=solution[:, P_ind]) 
        ...
        for i, species in enumerate(self.dynamic_species):
            df2[species] = np.concatenate([
                                ...
                                solution2[700:1000:30, i] # last 300 points, every 30th
                            ])
